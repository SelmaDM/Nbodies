...
if(rank == 0):
	...
	dataNew = split(data, size)
else:
	data = None
	dataNew = None

l_data = comm.scatter(dataNew, root = 0)
g_data = comm.bcast(data, root = 0)

for _ in range(NBSTEPS):
	force = [[0,0] for _ in range(0, len(l_data))]
	for i in range(0, len(force)):
		for j in range(nbbodies):
			[fx,fy] = force[i]
			[dfx, dfy] = interaction(l_data[i], g_data[j])
			force[i] = [fx+dfx, fy+dfy]
	for i in range(0, len(force)):
		l_data[i] = update(l_data[i], force[i])
	g_data = unsplit(comm.allgather(l_data))
	if(rank == 0):
		displayPlot(g_data)

end = MPI.Wtime()
if(rank == 0):
	print("Parallel signature: "+str(signature(g_data)))
